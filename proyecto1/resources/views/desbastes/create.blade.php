...
@section('content')
<div class="container">
    <h2 class="text-center mb-4" style="color: #444;">Agregar Operación de Desbaste</h2>

    {{-- Formulario para agregar operación de desbaste --}}
    <label style="color: red;">{{ __('Datos obligatorios *') }}</label><br><br>

    <div class="card shadow p-4" style="background-color: while; border-radius: 15px; border: 1.5px solid #FFCC00;">
        <form action="{{ route('desbastes.store') }}" method="POST" id="desbasteForm">
            @csrf
            <div class="row mb-3">
                <div class="col-md-6">
            ...
            <a href="{{ route('desbastes.index') }}" class="btn btn-secondary btn-block mt-4">Cancelar</a>
            </div></center>
        </form>
    </div>
...
<script>
    ...
    document.getElementById('desbasteForm').addEventListener('submit', function(event) {
